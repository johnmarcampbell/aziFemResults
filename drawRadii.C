...
TCanvas* makeRadiiCanvas(TDirectory* theDirectory, TGraphErrors** params, TString canvasName, TString paveText);
    ...
    for (Int_t iZdc = 0; iZdc <= (nZdcBins - 1); iZdc++)
    {
        ...
        for (Int_t iq2Mult = 0; iq2Mult <= 1; iq2Mult++)
        {
            ...
            for (Int_t i = 0; i <= (nQ2MultBins - 1); ++i)
            {
                ...
                TDirectory* theDirectory = inFile->GetDirectory(dirName.Data());
