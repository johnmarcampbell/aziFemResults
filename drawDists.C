...
void drawDists()
{
    ...
    TString outFileNameBase[4] = {"UUMultDists.pdf", "UUq2Dists.pdf", "AuAuMultDists.pdf", "AuAuq2Dists.pdf"};
...
TCanvas* makeCFCanvas(TFile* inFile, TString dirName, TString paveText)
{
    ...
    TH1D* paramsNew[6];
    TH1D* paramRatios[3];
    const TString paramNames[6] = {"RoutSquared_Dist", "RsideSquared_Dist", "RlongSquared_Dist", "RoutsideSquared_Dist", "Lambda_Dist", "Normalization_Dist", };
    const TString paramNamesNew[6] = {"RoutSquared_DistNew", "RsideSquared_DistNew", "RlongSquared_DistNew", "RoutsideSquared_DistNew", "Lambda_DistNew", "Normalization_DistNew", };
    ...
    for (Int_t i = 0; i <= 5; ++i)
    {
        ...
        paramsNew[i] = (TH1D*)gDirectory->Get(paramNamesNew[i].Data());
